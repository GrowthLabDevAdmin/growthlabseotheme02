...
// Render ACF blocks with a custom template if it exists in the child theme, otherwise use the parent theme template.
add_filter('acf/blocks/render_template', function ($template, $block) {
    $block_name = str_replace('acf/', '', $block['name']);
    $child_template = get_stylesheet_directory() . '/blocks/' . $block_name . '/block.php';
    $parent_template = get_template_directory() . '/blocks/' . $block_name . '/block.php';
...
//Render ACF blocks with a custom style if it exists in the child theme, otherwise use the parent theme style.
add_filter('acf/blocks/render_style', function ($style, $block) {
    $block_name = str_replace('acf/', '', $block['name']);
    $child_style = get_stylesheet_directory() . '/blocks/' . $block_name . '/block-min.css';
    $parent_style = get_template_directory() . '/blocks/' . $block_name . '/block-min.css';

    if (file_exists($child_style)) {
        return $child_style;
    } elseif (file_exists($parent_style)) {
        return $parent_style;
    }
...
//Render ACF blocks with a custom script if it exists in the child theme, otherwise use the parent theme script.
add_filter('acf/blocks/render_script', function ($script, $block) {
    $block_name = str_replace('acf/', '', $block['name']);
    $child_script = get_stylesheet_directory() . '/blocks/' . $block_name . '/block.js';
    $parent_script = get_template_directory() . '/blocks/' . $block_name . '/block.js';

    if (file_exists($child_script)) {
        return $child_script;
    } elseif (file_exists($parent_script)) {
...
// Render ACF blocks preview with a custom template if it exists in the child theme, otherwise use the parent theme template.
add_filter('acf/blocks/preview_template', function ($template, $block) {
    $block_name = str_replace('acf/', '', $block['name']);
    $child_template = get_stylesheet_directory() . '/blocks/' . $block_name . '/preview.php';
    $parent_template = get_template_directory() . '/blocks/' . $block_name . '/preview.php';
    ...
    if (file_exists($child_template)) {
        ...
        echo '<div class="acf-block-editor-placeholder">
                <span class="dashicons dashicons-layout"></span>
                <strong>' . esc_html($block['title']) . '</strong>
                <p>Edit the layout from the sidebar →</p>
              </div>';
        return;
    }
